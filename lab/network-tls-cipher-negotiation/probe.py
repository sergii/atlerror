...
SERVER_CIPHER = "ECDHE-RSA-AES256-GCM-SHA384"
INTERVENTION_CIPHER = "ECDHE-RSA-AES128-GCM-SHA256"

EXPERIMENT_ID = "experiment.network.tls_cipher_negotiation.python_linux"
CLAIM_ID = "claim.network.tls_cipher_negotiation_failure.no_common_cipher_blocks_handshake"
...
def attempt(cipher):
    ...
    result = {
        ...
        "client_tls_min": "TLSv1_2",
        "client_tls_max": "TLSv1_2",
...
baseline = attempt(SERVER_CIPHER)
intervention = attempt(INTERVENTION_CIPHER)
recovery = attempt(SERVER_CIPHER)
...
assertions = {
    ...
    "baseline_shared_cipher_negotiates": baseline.get("cipher") == SERVER_CIPHER,
    ...
    "configured_tls_versions_overlap": True,
    "configured_cipher_sets_do_not_overlap": SERVER_CIPHER != INTERVENTION_CIPHER,
    "recovery_tls12_establishes": recovery["tls_established"] and recovery.get("tls_version") == "TLSv1.2",
    "recovery_shared_cipher_negotiates": recovery.get("cipher") == SERVER_CIPHER,
...
evidence = {
    ...
    "intervention": {
        "action": "client_disjoint_cipher_against_tls12_server",
        "tls_server": TLS_SERVER,
        "tls_port": TLS_PORT,
        "server_name": SERVER_NAME,
        "trusted_ca_file": CA_FILE,
        "server_tls_versions": ["TLSv1.2"],
        "client_tls_versions": ["TLSv1.2"],
        "server_ciphers": [SERVER_CIPHER],
        "baseline_client_ciphers": [SERVER_CIPHER],
        "intervention_client_ciphers": [INTERVENTION_CIPHER],
    },
    "observations": {
        "baseline": baseline,
    ...
    "result": result,
    "interpretation": "The same reachable peer, TLS 1.2 protocol version, and trusted certificate succeed when client and server share ECDHE-RSA-AES256-GCM-SHA384. A TLS 1.2-only client restricted to ECDHE-RSA-AES128-GCM-SHA256 establishes TCP but cannot create a TLS session because the server permits only the disjoint AES256 cipher. Recovery succeeds after restoring the shared cipher.",
    "limitations": [
        "The cipher mismatch is intentionally configured in a disposable Docker lab.",
        "The experiment validates one TLS 1.2 cipher-suite incompatibility, not every TLS negotiation mechanism.",
        "It does not cover TLS 1.3 cipher configuration, protocol-version mismatch, signature algorithms, groups, ALPN, mTLS, certificate verification, or middlebox behavior.",
    ],
}
